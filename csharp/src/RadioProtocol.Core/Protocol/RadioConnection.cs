...
/// <summary>
/// High-level radio connection manager using IRadioTransport
/// </summary>
public sealed class RadioConnection : IDisposable
{
    ...
    public async Task<bool> InitializeAsync(CancellationToken ct = default)
    {
        ...
        _logger.LogInfo("No response from device within timeout");
        return false;
    }

    private async Task<bool> SendHandshake(CancellationToken ct)
    {
        var handshake = FrameFactory.Handshake();
        _logger.LogRawDataSent(handshake);
        return await _transport.WriteAsync(handshake);
    }

    public async Task<bool> SendAsync(CanonicalAction action)
    {
        var bytes = FrameFactory.Build(action);
        _logger.LogInfo($"Sending action: {action}");
        _logger.LogRawDataSent(bytes);
